  ...
  export async function resourceRegister(
    ...
    const { name, description, height, width, length, category_id } = request.body;
    ...
    try {
      await registerResourceService(company_id, { name, description, height, width, length, category_id});
    } catch (error) {
      console.log(error)
      return reply.status(409).send();
    }
